<script>
  ...
  const sheetId = '1XKu0aXxsW_sFp4r4z5gSwT70QJRsixA36cDcCB4FS6U';
  ...
  try {
    ...
    monsterNames = slicedValues.map(([name, value]) => ({ name, value }));
          ...
          {#each convertStringToArray(row.sub) as sub, subIndex}
            {#if (
              ...
              <div class="data-entry">
                ...
                {/if}
                <div class="row youtube-link">
                  <select bind:value={selectedName}>
                    {#each monsterNames as monster}
                      <option value={monster.value}>{monster.name}</option>
                    {/each}
                  </select>
                  <a class="youtube" href={`https://www.youtube.com/results?search_query=パズドラ${sub}${selectedName}`} target="_blank">
                    Link to YouTube
                  </a>
                </div>
                <p>if you dont have a specific leader in mind, leave it blank</p>
              </div>
            {/if}
          {/each}
        {/each}
      {:else}
        <p>No data available. Please type a valid dungeon name.</p>
      {/if}
    {/if}
